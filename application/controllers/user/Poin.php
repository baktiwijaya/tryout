...
#------------------------------------    
    ...
    public function load_table() {
        $data['list'] = $this->Crud_m->all_data('transaksi_poin', '*','id_user='.$this->session->userdata('id'));
        $this->load->view('user/poin/table', $data);
    }
    ...
    public function edit() {
        extract($_POST);
        $data['id'] = $id;
        $data['detail'] = $this->Crud_m->get_one('*', 'transaksi_poin', 'id_transaksi', $id);
        $this->load->view('user/poin/edit', $data);
    }

    public function save() {
        extract($_POST);
        $data = array(
            'id_paketpoin' => $id_paketpoin,
            'tanggal_pembelian' => date('Y-m-d H:i:s'),
            'id_user' => $this->session->userdata('id'),
            'status' => 0
        );

        $add = $this->Crud_m->add('transaksi_poin', $data);
        if ($add) {
